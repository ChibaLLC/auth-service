...
type FinderFunction<T> = (item: T, index: number, arr: T[]) => boolean;
...
export function deleteArrayItems<T>(
	...
	let predicate: (item: T, index: number, arr: T[]) => boolean;
	...
	switch (true) {
		...
		case typeof target === "object":
			if (Array.isArray(comp)) {
				...
				predicate = (item) => deepEqual(item as object, target);
			} else {
				predicate = (item) => {
		...
		case typeof target === "function":
			predicate = (item, index, arr) => (target as FinderFunction<T>)(item, index, arr);
			break;
		default:
			predicate = (item) => item === target;
			break;
	}
